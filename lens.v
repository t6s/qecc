...
exists (extract l2 (val l1)).
abstract (case: l1 l2 => l1' Hl1 [l2' Hl2] /=;
          rewrite map_inj_uniq ?enum_uniq // => i j /tnth_inj /tnth_inj; exact).
Defined.

Lemma index_lens_comp i (H : index i (val l1) < m) :
  index i (val lens_comp) = index (Ordinal H) (val l2).
Proof.
rewrite /=.
...
rewrite -[X in _ < X](size_tuple (val lens_comp)) index_mem.
